...
describe('Studio to avatar definition conversion', () => {
  it('exports the complete bundled document with curated semantic keys', () => {
    ...
    expect(result.ok).toBe(true)
    if (!result.ok) return
    expect(result.value.expressionOrder).toHaveLength(document.expressions.length + 1)
    expect(result.value.expressionOrder[0]).toBe('neutral')
    expect(result.value.animationOrder).toHaveLength(document.sequences.length)
