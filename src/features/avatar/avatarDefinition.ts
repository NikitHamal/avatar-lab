...
const normalizeExportEyeMotion = (motion?: string): 'none' | 'microSaccades' | 'shake' => {
  if (motion === 'microSaccades' || motion === 'shake') return motion
  return 'none'
}

const normalizeExportBodyMotion = (motion?: string): 'none' | 'slowDrift' | 'shake' => {
  if (motion === 'slowDrift' || motion === 'shake') return motion
  return 'none'
}
...
const mapExpression = (expression: Expression): AvatarExpressionDefinition => ({
  ...
  motion: {
    eyes: normalizeExportEyeMotion(expression.eyeMotion),
    body: normalizeExportBodyMotion(expression.bodyMotion),
  },
...
export const createAvatarDefinition = ({
  ...
  const definition: AvatarDefinition = {
    ...
    body: {
      primary: mapSurface(avatar.body.primary as unknown as SurfaceDefinition<SurfaceType>),
      nodes: avatar.body.nodes.map((node): AvatarBodyNodeDefinition => ({
        surface: mapSurface(node.surface as unknown as SurfaceDefinition<BodyNodeSurfaceType>),
