...
export type StudioLanguage = 'fr' | 'en' | 'zh-CN'
...
const english: Record<string, string> = {
  ...
  Fleur: 'Flower',
  Disque: 'Disc',
  'Bouche & personnalité': 'Mouth & personality',
  Bouche: 'Mouth',
  'Échelle de bouche': 'Mouth scale',
  Respiration: 'Breathing',
  Rebond: 'Bounce',
  Balancement: 'Sway',
  Flottement: 'Float',
  'Regard errant': 'Wandering gaze',
  'Balayage du regard': 'Look around',
  'Focus vivant': 'Living focus',
  'Rechercher un personnage': 'Search characters',
  Remix: 'Remix',
  'Hochement doux': 'Gentle bob',
  'Style de bouche': 'Mouth style',
  Aucune: 'None',
  Sourire: 'Smile',
  'Grand sourire': 'Grin',
  'Sourire ouvert': 'Open smile',
  Neutre: 'Neutral',
  Triste: 'Sad',
  Malicieux: 'Smirk',
  Chat: 'Cat',
  Surprise: 'Surprised',
  Bisou: 'Kiss',
  'Taille de bouche': 'Mouth size',
  'Activer la bouche': 'Enable mouth',
  'Bouche procédurale': 'Procedural mouth',
  'Bouche procédurale et options expressives.': 'Procedural mouth and expressive options.',
...
export const translateStudioText = (text: string, language: StudioLanguage) => {
  ...
  if (language === 'zh-CN') {
    const dynamicZh = translateChineseDynamicText(text)
    if (dynamicZh) return dynamicZh
    const exactZh = chinese[text]
...
export function StudioLanguageProvider({
  ...
  const [currentLanguage, setLanguage] = useState<StudioLanguage>(language)
  return createElement(
    StudioLanguageContext.Provider,
    {
      value: {
        ...
        setLanguage,
        t: text => translateStudioText(text, currentLanguage),
      },
    },
    children
  )
